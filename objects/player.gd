...
class FusionRifle:
	...
	var rounds_per_burst = 5;
	var difference_per_round = 5;
	var _round_iter = 0;
	var _fireIter = 0;
	var mx = 0;
	var my = 0;
	...
	func _ready():
		...
		ready = true
	func fire_check():
		mx = parent.get_global_mouse_position().x
		my = parent.get_global_mouse_position().y
		if(!ready):
			return
		var current_fire = Input.is_action_pressed("fire")
		var lifted_fire = Input.is_action_just_released("fire")
		
		#Checks if key pressed and 
...
#	healthbar.value = health
	...
	if Input.is_action_just_pressed("ui_accept"): #Test: Spawns a weapon drop
		...
		var weapon_drop2 = weapon_drop_scene.instance()
		var vectorPlace2 = Vector2(position.y, position.y) + Vector2(10,0).rotated(rotation)
		weapon_drop2.start(position.x - 100, position.y, FusionRifle.new("res://starterGun_bullet.tscn", self))
		weapon_drop2.set_name("weapon_drop2")
		get_parent().add_child(weapon_drop2)
